...
def analytics_view(request):
    ...
    # Filter sales based on the filter parameter
    if filter_by == 'hour':
        sales = Sale.objects.filter(date=today)
        sales_by_hour = SaleItem.objects.extra(select={'hour': "EXTRACT(HOUR FROM time)"}).values('hour').annotate(total_sales=Sum('price')).order_by('-total_sales')
        data = {
        ...
        data = {
            ...
            'yesterday_sales': yesterday_sales['total'] or 0,
        }
        ...
        data = {
            'year_sales': year_sales['total'] or 0,
        }

    # Best-selling meal
    best_selling_meal = SaleItem.objects.values('meal__name').annotate(total_sold=Sum('quantity')).order_by('-total_sold').first()
